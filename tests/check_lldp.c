...
/*
 * sport means send port
 * rport means receive port.
 */
static void
check_received_port(
...
/*
 * schassis means send chassis
 * rchassis means receive chassis.
 */
static void
...
START_TEST (test_send_rcv_basic)
{
	...
	/* Build packet */
	fprintf(stderr,"send to the interface \n");
	n = lldp_send(NULL, &hardware);
	if (n != 0) {
		fail("unable to build packet");
		return;
...
END_TEST
/*
 * hardware.h_lport.p_mfs means the msgFastTx(default 1(1~3600s)
 * msgTxInterval means transfer time ,default is 30s(1~3600s)
 */
